    ...
    <td><select name="sub_cat" id="sub_cat" required></select></td>
        ...
        <select name="sem" required>
          ...
          <option value="summer">summer</option>
			...
			<td><input type="submit" name="add" value="Generate"/></td>
		</tr>
...
<form action="" method="post">
  ...
  <select name="yrlvl">
    ...
    <?php 
      $yrlvl=mysql_query("SELECT DISTINCT yrlvl FROM schedules");
  ...
  <select name="sem">
    <option value="">--Semester--</option>
    <?php 
      $sem=mysql_query("SELECT DISTINCT sem FROM schedules ORDER BY sem ASC");
...
if(isset($find))
{
  ...
  {
$result = mysql_query("SELECT * FROM schedules,rooms,subject,course,sy,time WHERE schedules.room_id=rooms.room_id AND schedules.subject_id=subject.subject_id AND schedules.course_id=course.course_id AND schedules.sy_id=sy.sy_id AND schedules.time_id=time.time_id AND sy.sy_stat='active' AND schedules.course_id='$course' AND schedules.yrlvl='$yrlvl' AND schedules.sem='$sem' AND schedules.sy_id='$sy' ORDER BY schedules.section_id ASC");
...
<tr align="center">
  ...
  <td><?php echo $getresult['subject_units']?></td>
  <td><?php echo $getresult['yrlvl']?></td>
  <td><?php echo $getresult['section_id']?></td>
  <td><a style="text-decoration:none;" href="classtimeedit.php?sched_id=<?php echo $getresult['sched_id']?>&&rid=<?php echo $getresult['room_id']?>&&tid=<?php echo $getresult['time_id']?>"><?php echo $getresult['time_in'].'-'.$getresult['time_out'].''.$getresult['type'].' '.$getresult['monday'].' '.$getresult['tuesday'].' '.$getresult['wednesday'].' '.$getresult['thursday'].' '.$getresult['friday'].' '.$getresult['saturday'].' '.$getresult['sunday']?></a></td>
  <td><a style="text-decoration:none;" href="classroomedit.php?sched_id=<?php echo $getresult['sched_id']?>&&rid=<?php echo $getresult['room_id']?>&&tid=<?php echo $getresult['time_id']?>"><?php echo $getresult['room_code'].'/'.$getresult['room_description']?></a></td>
...
<!--Update status-->
<?php 
  mysql_query("UPDATE schedules SET status='full' WHERE pop = '$getresult[size]' ");
  } } } }else{   
?>
<tr align="center"><td colspan="12">No Records Found</td></tr>
		<?php }
    $ch=mysql_query("SELECT * FROM schedules WHERE `size`!=`pop` ");
    while($getch=mysql_fetch_array($ch)){
      //echo $getch['sched_id'];
    mysql_query("UPDATE schedules SET status='not full' WHERE sched_id = '$getch[sched_id]' ");
    }
