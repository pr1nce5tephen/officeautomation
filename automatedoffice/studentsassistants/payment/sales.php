...
<select name="contribution" style="width: 520px;">
	<?php
	include('../connect.php');
	$result = $db->prepare("SELECT * FROM contribution");
		$result->bindParam(':student_id', $res);
		$result->execute();
		for($i=0; $row = $result->fetch(); $i++){
	?>
		<option value="<?php echo $row['contribID']; ?>"><?php echo $row['contribDesc']; ?> - <?php echo $row['contribAmount']; ?></option>
	<?php
	}
	?>
</select>
